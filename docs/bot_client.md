...
- **محدودیت‌ها:**
  - بدون پشتیبانی از resume در آپلود/دانلود.
  - InlineMessage بدون چک تکراری (ممکن است تکراری پردازش شود).
  - Rate limit ساده (بدون پشتیبانی از burst).
- **پیشنهادات:**
  - افزودن retry با `tenacity`.
  - پشتیبانی از chunked upload.
  - بهبود `upload_file` با context manager.
